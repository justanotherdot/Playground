...
def find_def(query):
    ...
    if re.match(suffix, sys.argv[0]):
        print("You are attempting to find_def this script directly.")
        print("Please refind_def with 'define <word>'")
    else:
        ...
        for item in root.findall('.//'):
            # Dictionary names are under the 'Name' tag. Definitions are under
            # 'WordDefinition'.
            # Care should be made because the root is ALSO called
            # WordDefinition
            match = re.match(dict_name, item.tag) \
                or re.match(definition, item.tag)
            if match:
                print(item.text)
